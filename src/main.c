...
t_node *new;
  if(!(new = ft_memalloc(sizeof(t_node))))
    return;
new->name = ft_strdup(fil->name);
new->path = ft_strdup(fil->path);
new->fullname = ft_strdup(fil->fullname);
lstat(new->fullname, &(new->data));
new->isdir = (S_ISDIR(new->data.st_mode)) ? true : false;
  ...
  while (fil)
   {
     ...
     if ((flags & 0x2) && fil->isdir == true && ft_strcmp(fil->name, ".") != 0 && ft_strcmp(fil->name, "..") != 0)
...
while((dp = readdir (dir)) != NULL)
   {
     addnode(&fil, (*dirlist)->fullname,dp->d_name);
   }
